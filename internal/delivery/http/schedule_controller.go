...
func (c *ScheduleController) GetByStudentUserID(w http.ResponseWriter, r *http.Request) {
	...
	// Panggil UseCase
	response, err := c.UseCase.GetScheduleByStudentUserID(r.Context(), &request)
	if err != nil {
		c.Log.Printf("Failed to get schedule: %v", err)
		http.Error(w, err.Error(), helper.GetStatusCode(err))
		return
	...
	// Kirim response sukses
	if err := json.NewEncoder(w).Encode(model.WebResponse[*model.ScheduleStudentResponse]{Data: response}); err != nil {
		c.Log.Printf("Failed to write response: %v", err)
		http.Error(w, "Internal Server Error", http.StatusInternalServerError)
	}
}
